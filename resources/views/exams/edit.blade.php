...
<div class="mx-auto mt-4 max-w-3xl overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm">
    ...
    @endif
    <form method="POST" action="{{ route('exams.update', $exam) }}" class="space-y-6 px-6 py-6">
        @csrf
        @method('PUT')
        <div>
            <label for="title" class="mb-1 block text-sm font-medium text-gray-700">Titre de l'examen</label>
            <input type="text" name="title" id="title" class="w-full rounded-lg border-gray-300 bg-gray-50 px-3 py-2 text-sm focus:border-primary focus:ring-primary/20" required value="{{ old('title', $exam->title) }}">
        </div>
        <div>
            <label for="description" class="mb-1 block text-sm font-medium text-gray-700">Description</label>
            <textarea name="description" id="description" rows="4" class="w-full rounded-lg border-gray-300 bg-gray-50 px-3 py-2 text-sm focus:border-primary focus:ring-primary/20">{{ old('description', $exam->description) }}</textarea>
        </div>
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
            ...
            <div>
                ...
                <input type="date" name="exam_date" id="exam_date" class="w-full rounded-lg border-gray-300 bg-gray-50 px-3 py-2 text-sm focus:border-primary focus:ring-primary/20" required value="{{ old('exam_date', $exam->exam_date) }}">
            </div>
        </div>
        <div class="flex justify-end">
            <button type="submit" class="rounded-button bg-primary px-5 py-2.5 font-medium text-white hover:bg-primary/90">Enregistrer</button>
        </div>
    </form>
</div>
@endsection
