...
<div class="bg-white rounded-lg shadow-sm overflow-hidden max-w-xl mx-auto mt-8">
    ...
    <form method="POST" action="{{ route('exams.store') }}" class="px-6 py-4">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="title" id="title" class="form-control w-full" required value="{{ old('title') }}">
        </div>
        <div class="mb-4">
            <label for="description" class="block font-medium">Description</label>
            <textarea name="description" id="description" class="form-control w-full">{{ old('description') }}</textarea>
        </div>
        <div class="mb-4">
        ...
        <div class="mb-4">
            ...
            <input type="date" name="exam_date" id="exam_date" class="form-control w-full" required value="{{ old('exam_date') }}">
        </div>
        <button type="submit" class="px-4 py-2 bg-primary text-white font-medium rounded-button hover:bg-primary/90">Créer l'examen</button>
    </form>
</div>
@endsection
