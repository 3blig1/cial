    ...
    <div class="flex w-full flex-col gap-3 sm:w-auto sm:flex-row sm:flex-wrap sm:items-center sm:justify-end">
        <form method="GET" action="{{ route('students.index') }}" class="w-full sm:w-auto">
            <div class="relative w-full sm:w-80">
                ...
                <input type="search" name="search" value="{{ request('search') }}" placeholder="Rechercher un élève..." class="pl-10 pr-4 py-2 w-full rounded-lg border-gray-200 bg-gray-50 focus:ring-2 focus:ring-primary/20 text-sm">
            </div>
        </form>
        <a href="{{ route('students.create') }}" class="px-4 py-2 bg-primary text-white font-medium rounded-button hover:bg-primary/90 flex items-center justify-center gap-2">
            <i class="ri-add-line"></i>
            <span>Ajouter un élève</span>
        </a>
        <a href="{{ route('pending-students.index') }}" class="px-4 py-2 bg-yellow-500 text-white font-medium rounded-button hover:bg-yellow-600 flex items-center justify-center gap-2">
